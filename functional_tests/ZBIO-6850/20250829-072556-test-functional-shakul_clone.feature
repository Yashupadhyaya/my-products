...
roost_feedback [29/08/2025, 11:24:17 AM]:add\ssome\smore\sscenarios\splease
*/
...
Scenario: כניסה לאפליקציה באמצעות טוקן
  Given האדרס הבסיסי של ה-API הוא '${BASE_URL}'
  When שליחת בקשת POST ל '/token-login' עם גוף הבקשה:
  """
  {
    "token": "token123"
  }
...
Scenario: נעילה אוטומטית לאחר 5 ניסיונות כושלים
  Given האדרס הבסיסי של ה-API הוא '${BASE_URL}'
  When שליחת בקשת POST ל '/login' עם גוף הבקשה:
  """
  {
...
Scenario: צפייה בפרטי חשבון של מוטב
  Given האדרס הבסיסי של ה-API הוא '${BASE_URL}'
  When שליחת בקשת GET ל '/beneficiary-account-details'
  Then הקוד של התגובה צריך להיות 200
  And הגוף של התגובה צריך להכיל 'יתרה כללית', 'פירוט עסקאות', 'מספר חשבון', 'מזהה סניף'
...
Scenario: העברות כספים לחשבון חיצוני עם קוד אימות
  Given האדרס הבסיסי של ה-API הוא '${BASE_URL}'
  When שליחת בקשת POST ל '/external-transfer' עם גוף הבקשה:
  """
  {
